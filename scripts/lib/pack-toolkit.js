...
function createPackReport(rootDirectory, packId) {
    ...
    const chapters = loaded.manifest.chapters
        .map(chapter => {
            const levels = audit.levels.filter(level => level.chapterId === chapter.id);
    ...
    const scores = audit.levels.map(level => level.score).filter(Number.isFinite);
    ...
    for (let index = 1; index < audit.levels.length; index++) {
        ...
        adjacentChanges.push({
            ...
            scoreDelta: Number.isFinite(previous.score) && Number.isFinite(current.score)
...
function renderPackReportHtml(report) {
    const rows = report.levels.map(level => `<tr class="${level.valid ? '' : 'invalid'}">
...
<td>${level.valid ? '通过' : escapeHtml(level.errors.join('；'))}</td>
    ...
    const chapterRows = report.chapters.map(chapter => `<tr>
...
function diffPacks(rootDirectory, leftRef, rightRef) {
    ...
    const leftLevels = new Map(left.resolved.levels.map(level => [stableLevelId(level), level]));
    const rightLevels = new Map(right.resolved.levels.map(level => [stableLevelId(level), level]));
    const added = [];
    const removed = [];
    const changed = [];
...
function createStarterPack(rootDirectory, packId, options = {}) {
    ...
    if (fs.existsSync(directory)) throw new Error(`Pack directory already exists: ${directory}`);
    ...
    if (!schemas.manifest(manifest) || !schemas.presets(presets) || !schemas.levels(levels)) {
...
function loadPackReference(rootDirectory, reference) {
    ...
    createRuntimeValidator(root).validateBundle(entry, manifest, presets, levelList);
    const runtime = createGameRuntime(root);
    return {
        manifest,
        presets,
        levelList,
        resolved: new runtime.LevelResolver().resolvePack(manifest, presets, levelList)
    };
}
...
function createGameRuntime(root) {
    ...
    loadScript(root, context, 'js/managers/DifficultyManager.js', ['DifficultyManager']);
    loadScript(root, context, 'js/managers/DifficultyModelV2.js', ['DifficultyModelV2']);
...
function summarizeCapacity(capacity) {
    return {
        theoretical: finiteOrNull(capacity.layoutTheoretical ?? capacity.theoretical),
        comfortable: finiteOrNull(capacity.layoutComfortable ?? capacity.comfortable),
...
function changedFields(left, right) {
    const fields = [
        ...
        ['presentation', left.presentation || left.designIntent, right.presentation || right.designIntent],
