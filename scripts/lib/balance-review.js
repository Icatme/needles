...
const {
    createPackReport
} = require('./pack-toolkit');

function buildBalanceReview(rootDirectory, campaignPath, sources, options = {}) {
    ...
    const analysisPack = analysis.packs.find(pack => (
        pack.packId === loaded.campaign.packId
    ...
    const anchors = loaded.campaign.anchors.map(anchor => {
        ...
        const maxAdjacentFailureDelta = maximumFinite([
            subtractNullable(
                actual?.failureRate,
                previousActual?.failureRate
            ),
            subtractNullable(
                nextActual?.failureRate,
                actual?.failureRate
            )
        ...
        return {
            ...
            neighbors: {
                previous: previous
                    ...
                    : null,
                next: next
                    ? summarizeNeighbor(next, nextActual, predictedById)
                    : null
            }
        };
    });
...
function summarizeLevelParameters(level) {
    ...
    return {
        ...
        shotModifier: level.rhythm?.shotModifier
            ...
            : null,
        presentation: JSON.parse(JSON.stringify(
            level.presentation || level.designIntent || {}
        )),
        tags: [...(level.tags || [])]
    };
}
...
function compareCollection(a, b) {
    ...
    return readinessA - readinessB
        || (b.gaps.attempts + b.gaps.sources)
...
function compareReview(a, b) {
    return nullableForDescending(b.transparentSort.absoluteRankDelta)
        ...
        || nullableForDescending(b.transparentSort.maximumAdjacentFailureRateDelta)
            - nullableForDescending(a.transparentSort.maximumAdjacentFailureRateDelta)
...
function renderBalanceReviewHtml(review) {
    const collectionRows = review.collectionQueue.map(item => `<tr>
...
<td><a href="${escapeHtml(item.previewURL)}">继续采集</a></td></tr>`).join('\n');
    const reviewRows = review.reviewQueue.map(item => `<tr>
...
<td><a href="${escapeHtml(item.previewURL)}">复查</a></td></tr>`).join('\n');
