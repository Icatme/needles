...
function loadCampaign(rootDirectory, campaignPath) {
    ...
    const levelsById = new Map(
        pack.resolved.levels.map(level => [level.packLevelId || String(level.id), level])
    );
    const seenOrders = new Set();
    campaign.anchors.forEach(anchor => {
        const level = levelsById.get(anchor.levelId);
        if (!level) throw new Error(`Campaign anchor is missing from pack: ${anchor.levelId}`);
...
function planCampaign(loaded, options = {}) {
    ...
    const skinId = options.skinId
        || loaded.campaign.defaultSkinId
        || null;
    const comparisonByOrder = loaded.comparison
        ? new Map(loaded.comparison.resolved.levels.map(level => [level.order, level]))
        : new Map();
...
function evaluateCampaign(loaded, sources) {
    ...
    const anchors = loaded.campaign.anchors.map(anchor => {
        ...
        return {
            ...
            status: pilotReady
                ? 'pilot-ready'
                : (matching.length === 0 ? 'unstarted' : 'collecting'),
            pilotReady,
...
function renderCampaignHtml(plan, status = null) {
    ...
    const rows = plan.anchors.map(anchor => {
