...
```text
<packId>@<packVersion>:<levelId>:score-v3:<ruleHash>
```

`ruleHash` 由最终归一化的评分规则生成。关卡包版本或评分规则变化时，
档案标识也会变化，旧公式下的本机成绩和幽灵不会与新公式混用。
V3 同时把连击算法纳入新的契约代号，避免 V2 的“普通成功即可续连”成绩与精准连击成绩混排。
...
- 第一针开启连击窗口；
- 下一针必须在窗口结束前获得单侧贴边或双侧穿隙判定，连击才会增加；
- 普通成功插针不会增加连击，而是把已有连击重置为 `×1`；
- 到达截止时刻即视为超时；超时后连击归零，下一针从 `×1` 重新开始；
- 单侧贴边会获得半段精准宽限；
- 双侧穿隙会获得完整精准宽限；
- 超时不会扣除已经获得的分数。
