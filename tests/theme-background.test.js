...
function createScene() {
	...
	return {
		...
		add: {
			container(x, y) {
				return createDisplayObject("container", x, y);
			},
			graphics() {
...
test("the two theme backgrounds use different structural languages", () => {
	...
	const observatoryCommands = observatoryScene.graphics.flatMap(
		(item) => item.commands,
	);
	...
	assert.equal(
		observatoryCommands.some(([name]) => name === "fillTriangle"),
		false,
	);
	assert.equal(
		jewelCommands.filter(([name]) => name === "fillTriangle").length,
		4,
	);
	assert.equal(
		observatoryCommands.some(
			(command) => command[0] === "fillCircle" && command[1] === 58,
		),
		true,
	);
	assert.equal(
...
test("scene shutdown leaves tween cleanup to Phaser and destroys the background tree", () => {
	...
	assert.equal(
		scene.tweenConfigs.every((tween) => tween.removed === false),
		true,
	);
});
...
test("game-over backgrounds stop ambient animation and add a result treatment", () => {
	...
	assert.equal(
		commands.some(
			(command) => command[0] === "fillStyle" && command[1] === 0x081015,
		),
		true,
	);
});
