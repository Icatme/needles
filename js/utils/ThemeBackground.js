class ThemeBackground {
    ...
    static drawObservatory(scene, root, mode) {
        const layout = LayoutManager.getBackgroundLayout(mode);
        const modeLayout = layout.mode;
        const focusY = modeLayout.focusY;
        ...
        base.fillEllipse(300, modeLayout.topGlowY, 460, 180);
        base.fillStyle(0x1d3a42, 0.06);
        base.fillEllipse(300, modeLayout.topGlowY, 360, 125);
        ...
        halo.strokeCircle(300, focusY, 160);
        halo.lineStyle(24, 0x355c63, 0.06);
        halo.strokeCircle(300, focusY, 138);
        halo.lineStyle(14, 0x355c63, 0.09);
        halo.strokeCircle(300, focusY, 116);
        if (mode === 'menu') {
            halo.lineStyle(30, 0x355c63, 0.05);
            halo.strokeCircle(300, focusY, 154);
        }

        const instruments = ThemeBackground.addGraphics(scene, root);
        instruments.lineStyle(2, 0xa67c39, mode === 'menu' ? 0.21 : 0.18);
        ThemeBackground.strokeArc(instruments, 300, focusY, 158, 3.48, 5.93);
        instruments.lineStyle(1, 0x4e7b73, mode === 'menu' ? 0.16 : 0.14);
        ThemeBackground.strokeArc(instruments, 300, focusY, 182, 3.3, 6.18);
        instruments.lineStyle(1, 0xa67c39, 0.12);
        ThemeBackground.strokeArc(instruments, 300, focusY, 214, 2.78, 3.92);
        ThemeBackground.strokeArc(instruments, 300, focusY, 214, 5.5, 6.62);
        instruments.lineStyle(1, 0xc89c52, 0.16);
        for (let index = 0; index < 24; index++) {
            const angle = index / 24 * Math.PI * 2;
            instruments.lineBetween(
                300 + Math.cos(angle) * 174,
                focusY + Math.sin(angle) * 174,
                300 + Math.cos(angle) * 182,
                focusY + Math.sin(angle) * 182
            );
        }
        instruments.lineStyle(2, 0xa67c39, 0.1);
        instruments.lineBetween(72, focusY - 80, 148, focusY + 140);
        instruments.lineStyle(2, 0xa67c39, 0.08);
        instruments.lineBetween(528, focusY - 100, 460, focusY + 125);

        const quietZones = ThemeBackground.addGraphics(scene, root);
        const quietHeight = Math.max(0, modeLayout.quietEndY - modeLayout.quietStartY);
        quietZones.fillStyle(0x0c1115, 0.3);
        quietZones.fillRect(252, modeLayout.quietStartY, 96, quietHeight);
        quietZones.lineStyle(1, 0x4e7b73, 0.1);
        quietZones.lineBetween(248, modeLayout.quietStartY + 32, 248, modeLayout.quietEndY);
        quietZones.lineBetween(352, modeLayout.quietStartY + 32, 352, modeLayout.quietEndY);
        ...
        if (mode === 'game-over') {
            ...
            result.strokeCircle(300, focusY, 124);
        }
    ...
    static drawJewelBox(scene, root, mode) {
        ...
        base.strokeRoundedRect(
            layout.innerFrame.x,
            layout.innerFrame.y,
            layout.innerFrame.width,
            layout.innerFrame.height,
            layout.innerFrame.radius
        );
        base.fillStyle(0x4a2034, 0.16);
        base.fillEllipse(300, modeLayout.topGlowY, 390, 150);
        base.fillStyle(0xe3b8b3, 0.08);
        base.fillEllipse(300, modeLayout.topGlowY + 30, 280, 90);
        base.fillStyle(0x4a2034, 0.18);
        base.fillEllipse(300, focusY, 270, 200);

        const cradle = ThemeBackground.addGraphics(scene, root);
        cradle.lineStyle(28, 0xd7b27a, 0.04);
        cradle.strokeCircle(300, focusY, 150);
        cradle.lineStyle(18, 0xd7b27a, 0.07);
        cradle.strokeCircle(300, focusY, 132);
        cradle.lineStyle(10, 0xc38a89, 0.09);
        cradle.strokeCircle(300, focusY, 116);
        if (mode === 'menu') {
            cradle.lineStyle(28, 0xd7b27a, 0.05);
            cradle.strokeCircle(300, focusY, 154);
        }
        ...
        const quietZones = ThemeBackground.addGraphics(scene, root);
        const sideHeight = Math.max(0, modeLayout.sideEndY - modeLayout.sideStartY);
        quietZones.fillStyle(0x9a6b57, 0.1 * trimMultiplier);
        quietZones.fillRect(34, modeLayout.sideStartY, 12, sideHeight);
        quietZones.fillRect(554, modeLayout.sideStartY, 12, sideHeight);
        const quietHeight = Math.max(0, modeLayout.quietEndY - modeLayout.quietStartY);
        quietZones.fillStyle(0x22101d, 0.34);
        quietZones.fillRoundedRect(248, modeLayout.quietStartY, 104, quietHeight, 30);
        quietZones.fillStyle(0x4a2034, 0.16);
        quietZones.fillEllipse(300, modeLayout.quietStartY + 47, 90, 70);
        quietZones.lineStyle(1, 0xc38a89, 0.12);
        quietZones.lineBetween(248, modeLayout.quietStartY + 28, 248, modeLayout.quietEndY - 15);
        quietZones.lineBetween(352, modeLayout.quietStartY + 28, 352, modeLayout.quietEndY - 15);
        ThemeBackground.drawBackings(quietZones, {
            hud: 0x201018,
            hudAlpha: 0.58,
            divider: 0xd7b27a,
    ...
    static drawBackings(graphics, palette, layout) {
        graphics.fillStyle(palette.hud, palette.hudAlpha);
        graphics.fillRect(0, 0, CONSTANTS.WIDTH, layout.topHudHeight);
        graphics.lineStyle(1, palette.divider, palette.dividerAlpha);
        graphics.lineBetween(
            0,
            layout.topHudHeight - 1,
            CONSTANTS.WIDTH,
            layout.topHudHeight - 1
        );
        const footer = layout.footerPanel;
